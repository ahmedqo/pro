...
<body class="bg-gray-700">
    <h1 class="text-white text-2xl pb-4">Modifier Devoir</h1>
            ...
            <div class="flex flex-col gap-4">
                <div class="flex flex-col md:flex-row gap-4">
                    <div class="flex-1 flex flex-col gap-2">
                        ...
                        <input id="title" type="text" placeholder="Titre" value="{{$devoirs->nom_tp}}" name="nom_tp" class="w-full bg-gray-700 text-white px-4 py-2 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600" />
                    </div>
                    <div class="flex-1 flex flex-col gap-2">
                        <label class="block text-white" for="date">Date limite</label>
                        <input id="date" type="date" placeholder="Date limite" value="{{$devoirs->date_limite}}" name="date_limite" class="w-full bg-gray-700 text-white px-4 py-2 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600" />
                    </div>
                </div>
                <div class="flex flex-col md:flex-row gap-4">
                    <div class="flex-1 flex flex-col gap-2">
                        ...
                        <select id="module" placeholder="Module" name="module" class="w-full bg-gray-700 text-white px-4 py-2 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600">
                            ...
                            <option selected disabled>{{ $modulename->nom_module }}</option>
                            @foreach ($data as $row)
                ...
                <div class="flex flex-col md:flex-row gap-4">
                    <div class="flex-1 flex flex-col gap-2">
                        ...
                        <textarea id="description" placeholder="{{$devoirs->description}}" value="{{$devoirs->description}}" name="description" class="w-full bg-gray-700 text-white px-4 py-2 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600"></textarea>
                    </div>
                </div>
                <div class="flex flex-col md:flex-row gap-4">
                    <div class="flex-1 flex flex-col gap-2">

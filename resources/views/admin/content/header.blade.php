			...
			<nav class="bg-gray-800">
				<div class="max-w-7xl mx-auto px-4">
					<div class="flex items-center justify-between md:justify-end h-14">
						<div class="flex items-center gap-2">
							<!-- header button icon start -->
							<button onclick="trigger(event)" data-target="sidebar" type="button" class="bg-gray-800 md:hidden cursor-pointer flex text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white" id="user-menu-button" aria-expanded="false" aria-haspopup="true">
								<svg class="w-6 h-6 text-gray-400 hover:text-white cursor-pointer pointer-events-none" fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000.000000 1000.000000">
							...
							<a href="https://flowbite.com" class="flex items-center md:hidden">
								<span class="self-center text-xl font-bold text-2xl whitespace-nowrap text-white">School Management</span>
							</a>
							<!-- header logo end -->
						</div>
						<div class="flex">
							<div class="relative">
								<div>
									...
									@endphp
									<!-- header button icon start -->
									<button onclick="trigger(event)" data-target="menu" type="button" class="trigger bg-gray-800 cursor-pointer flex text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white">
										<img class="w-8 h-8 rounded-full pointer-events-none object-cover" src="{{(!empty($data->profile_photo_path))? url('upload/profilepic/'.$data->profile_photo_path):url('upload/no_image.jpg')}}" />
									</button>
									<!-- header button icon end -->
								</div>
								<!-- header menu start -->
								<div id="menu" class="origin-top-right mt-4 absolute right-0 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 p-1 focus:outline-none hidden" role="menu">
									<a href="{{ route('admin.profile') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-200 rounded-md">Profile</a>
									<a href="{{ route('admin.settings') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-200 rounded-md">Settings</a>
									<a href="{{ route('admin.logout') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-200 rounded-md">Sign out</a>
								</div>
								<!-- header menu end -->
							</div>
						</div>
